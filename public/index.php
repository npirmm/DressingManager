...
// --- Composer Autoloader ---
// This line includes the Composer autoloader which knows how to find PHPMailer
...
// --- Pour tester l'envoi de mail ($route) ---
	case '/test-email':
		if ($requestMethod === 'GET') {
			$mailer = new Mailer();
			$recipient = 'test@example.com'; // Change to a real address you can check (or use Mailtrap)
			$subject = 'Test Email from Dressing Manager';

			// Option 1: Simple HTML string
			// $htmlBody = '<h1>Hello!</h1><p>This is a test email sent using PHPMailer.</p>';
			// $plainBody = "Hello!\nThis is a test email sent using PHPMailer.";

			// Option 2: Render a view (Create src/Views/emails/test.php first)
			try {
				$data = ['name' => 'Test User'];
				$htmlBody = $mailer->renderView('test', $data); // Assuming src/Views/emails/test.php exists
				 $plainBody = "Hello Test User!\nThis is a test from the template."; // Simple plain text version

				if ($mailer->send($recipient, $subject, $htmlBody, $plainBody)) {
					echo "Test email sent successfully to $recipient!";
				} else {
					echo "Failed to send test email.";
				}
			} catch (\Exception $e) {
				echo "Error preparing test email: " . $e->getMessage();
			}
